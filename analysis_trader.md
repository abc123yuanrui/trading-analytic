...
### 7.1 Portfolio Source of Truth
*   **Source:** The analyst MUST retrieve the current portfolio state (Assets, Units, and Cash) from the **"0. Portfolio Confirmation"** section of the most recent `daily_report_YYYY-MM-DD.md` file.
*   **Fallback:** If no daily report exists, refer to the most recent entry in `performance_log.csv`.
...
### 7.3 Rule of Sizing
- Always prioritize **Risk-Adjusted Units**. If the $83.92 is the only buffer, avoid putting 100% of it into a single volatile small-cap.  
- For PMGOLD, consider it a **Macro Hedge**. If global risk (e.g., U.S.-Iran tensions) is de-escalating, recommend "Hold" or "Trim" rather than aggressive "Buy."

### 7.4 Watch List
*   **BHP (BHP Group Ltd):** Focus on copper demand and China recovery catalysts.  
